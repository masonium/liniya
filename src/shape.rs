...
use super::common::*;
use ncollide3d::{query::{RayCast, Ray}, bounding_volume::AABB};
...
/// shape.
///
/// So far, there's nothing necessarily connects the paths with the
/// shape. One could use the shape intersection as a tie-breaker to
/// resolve potential edge-cases.
pub trait Shape {
    /// Ray-intersection
...
impl LatSphere {
	...
	LatSphere { transform, pos: *pos, shape: ball, radius, spacing_angle }
    }

    pub fn intersect(&self, ray: &Ray<f64>, max_toi: f64) -> Option<Intersection> {
	self.shape.toi_with_ray(&self.transform, ray, max_toi, true)
	    .map(|t| Intersection{ t, point: ray.point_at(t) })
    }
